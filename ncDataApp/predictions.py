...
minio_access_key = 'efHg5FLfSsLE6bGPrRmx'
minio_secret_key = 'scTEatGeWWQzgrCsk23pYSESoZrppV7KiN8oQBwL'
minio_endpoint = 'http://192.168.1.14:9000'  # Replace with your MinIO server endpoint

# Specify your MinIO bucket name
bucket_name = 'backend'

# Specify the object key for ERA5 and CAM6 data
file_key = 'ERA5/ERA5_SM_0_5m_0_45_leadday_1999_2021_organize_normalized.nc'
file_key1 = '/Nc_data/CAM6_SM_0_5m_0_45_leadday_1999_2021_organize_Ens_mean_normalized.nc'
...
with s3.open(f"{bucket_name}/{file_key1}", mode='rb') as s3_file1:
    data_cam6 = xr.open_dataset(s3_file1, engine='scipy')

# Read ERA5 file
with s3.open(f"{bucket_name}/{file_key}", mode='rb') as s3_file:
    data_Era5 = xr.open_dataset(s3_file, engine='scipy')
